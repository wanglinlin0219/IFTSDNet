...
Please cited:
L. Wang, J. Zhang, Q. Guo and D. Chen, "IFTSDNet: An Interact-Feature Transformer Network With Spatial Detail Enhancement Module for Change Detection," IEEE Geosci.
Remote Sens. Lett., vol. 20, pp. 1-5, 2023.
Q. Guo, J. Zhang, S. Zhu, C. Zhong, and Y. Zhang. "Deep Multiscale Siamese Network with Parallel Convolutional Structure and Self-Attention for Change Detection" IEEE Trans.
Geosci. Remote Sens., vol. 60, pp. 1-12, 2022.
M. Liu, Z. Chai, H. Deng and R. Liu, "A CNN-Transformer Network With Multiscale Context Aggregation for Fine-Grained Cropland Change Detection," Int.
J. Appl. Earth Obs. Geoinformation, vol. 60, pp. 1–16, 2022.
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epochs):
        ...
        model.train()
        logging.info('IFTSDNet model training!!!')
        batch_iter = 0
        ...
        for batch_img1, batch_img2, labels in tbar:
            ...
            # labels = labels[:, :, :, 0]   # If it is the DSIFN dataset, this line of code is needed.
            optimizer.zero_grad()


            cd_preds1, cd_preds2, cd_preds3 = model(batch_img1, batch_img2)
            cd_loss = criterion(cd_preds1, labels) + criterion(cd_preds2, labels) + criterion(cd_preds3,labels)
            ...
            cd_corrects = (100 *
                           (cd_preds1.squeeze().byte() == labels.squeeze().byte()).sum() /
                           (labels.size()[0] * (opt.patch_size**2)))

            cd_train_report = prfs(labels.data.cpu().numpy().flatten(),
                                   cd_preds1.data.cpu().numpy().flatten(),
                                   average='binary',
